...
namespace OpenSim.Tools.GuiControlPanel.Services
{
    /// <summary>
    ...
    public class OpenSimManager
    {
        ...
        public List<SimInstance> GetSimInstances()
        {
            ...
            foreach (var kvp in _runningInstances)
            {
                ...
                instances.Add(new SimInstance
                {
                    ...
                    StartTime = process.HasExited ? DateTime.MinValue : process.StartTime,
        ...
        public async Task<bool> StartSimAsync(string simName, string configPath = null)
        {
            try
            {
                ...
                {
                    FileName = Path.Combine(_openSimPath, "OpenSim.exe"),
                    Arguments = "",
        ...
        private List<string> GetConfiguredSimulations()
        {
            ...
            var configPaths = new[]
            {
                Path.Combine(_openSimPath, "bin", "Regions"),
                Path.Combine(_openSimPath, "bin", "config-include")
            };
        ...
        private string GetConfigPath(string simName)
        {
            var configPaths = new[]
            {
                Path.Combine(_openSimPath, "bin", "Regions", $"{simName}.ini"),
                Path.Combine(_openSimPath, "bin", "config-include", $"{simName}.ini"),
                Path.Combine(_openSimPath, "bin", $"{simName}.ini")
            };
